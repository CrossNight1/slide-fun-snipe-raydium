...
pub async fn parse_serum_market(rpc_client: &RpcClient, market_id: &Pubkey) -> Option<SerumMarketData> {
    ...
    for attempt in 0..5 {
        match rpc_client.get_account(market_id).await {
            Ok(account) => {
                ...
                // [317..349] asks               ← asks
                if data.len() < 350 {
                    log_info!("   [WARN] Serum market data too small: {} bytes (need 350+)", data.len());
                    return None;
                }
                ...
                return Some(SerumMarketData { bids, asks, event_queue, coin_vault, pc_vault, vault_signer_nonce });
            }
            Err(_) => {
                if attempt < 4 { sleep(Duration::from_millis(20)).await; }
            }
        }
    }
    None
...
async fn build_pool_info_from_accounts(
    ...
    if debug {
        ...
        log_info!("   [DEBUG] AMM ix candidate: accounts={}, data_len={}, disc={}", accounts.len(), data.len(), disc);
    }
    // initialize2 discriminator = 1
    if data.is_empty() || data[0] != 1 { return None; }
    if accounts.len() < 18 {
        log_info!("   [DEBUG] AMM V4 only {} accounts (need 18+)", accounts.len());
        return None;
    }
    ...
    let pool_sol_amount = if data.len() >= 26 {
        ...
        if base_mint == wsol { init_coin } else { init_pc }
    } else { 0 };
    ...
    let (market_bids, market_asks, market_event_queue,
         market_coin_vault, market_pc_vault, market_vault_signer) =
        if accounts.len() >= 24 {
            log_info!("   [+] Serum accounts from TX (fast path, no extra RPC)");
            (accounts[18], accounts[19], accounts[20],
            ...
            if let Some(data) = parse_serum_market(rpc_client, &market_id).await {
                let vault_signer = derive_vault_signer(&market_id, data.vault_signer_nonce, &serum_program);
                (data.bids, data.asks, data.event_queue,
                 data.coin_vault, data.pc_vault, vault_signer)
            } else {
...
pub async fn get_pool_info(rpc_client: &RpcClient, signature: &str) -> Option<PoolInfo> {
    ...
    for attempt in 0..10 {
        match rpc_client.get_transaction_with_config(&sig, config.clone()).await {
            Ok(tx) => {
                if let Some(decoded) = tx.transaction.transaction.decode() {
                    ...
                    for ix in instructions {
                        ...
                        if program_id != amm_program { continue; }
                        let mut accounts: Vec<Pubkey> = Vec::with_capacity(ix.accounts.len());
                        for idx in &ix.accounts {
                            let i = *idx as usize;
                            if i >= all_account_keys.len() { accounts.clear(); break; }
                            accounts.push(all_account_keys[i]);
                        }
                        if accounts.is_empty() { continue; }
                        if let Some(p) = build_pool_info_from_accounts(rpc_client, &accounts, &ix.data).await {
                            return Some(p);
                        }
                    }
                                ...
                                for ui_ix in &inner_ixs.instructions {
                                    match ui_ix {
                                        UiInstruction::Compiled(c) => {
                                            let program_id = all_account_keys[c.program_id_index as usize];
                                            if debug {
                                                log_info!(
                                            ...
                                            let mut accounts: Vec<Pubkey> = Vec::with_capacity(c.accounts.len());
                                            ...
                                            if accounts.is_empty() { continue; }
                                            if let Some(p) = build_pool_info_from_accounts(rpc_client, &accounts, &data).await {
                                                return Some(p);
                                            }
                                        }
                                        UiInstruction::Parsed(UiParsedInstruction::PartiallyDecoded(p)) => {
                                            let program_id = Pubkey::from_str(&p.program_id).ok()?;
                                            if debug {
                                                log_info!(
                                            ...
                                            let mut accounts: Vec<Pubkey> = Vec::with_capacity(p.accounts.len());
                                            for a in &p.accounts {
                    ...
                    if std::env::var("DEBUG_POOL_PARSE").ok().as_deref() == Some("1") {
                        log_info!("   [DEBUG] decode() returned None (attempt {})", attempt + 1);
                    }
            ...
            Err(_) => {
                if std::env::var("DEBUG_POOL_PARSE").ok().as_deref() == Some("1") {
                    log_info!("   [DEBUG] get_transaction_with_config error (attempt {})", attempt + 1);
                }
                if attempt < 9 { sleep(Duration::from_millis(10)).await; }
            }
        }
    }
    None
