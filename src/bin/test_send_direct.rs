...
async fn main() {
    ...
    let pool_info = match get_pool_info(&rpc, &args[1]).await {
        Some(p) => { println!("AMM: {} | Token: {}", p.amm_id, p.base_mint); p }
    ...
    let tx = match bundle_buy::build_raydium_buy_tx_for_wallet(
        ...
        config.cu_limit, config.priority_fee, bh,
    ) {
        Some(t) => t,
        None => { eprintln!("Could not build TX"); std::process::exit(1); }
    };
