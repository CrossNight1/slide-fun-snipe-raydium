...
fn bar(size: usize) -> String {
    ...
    format!(
        "[{}{}]",
        "█".repeat(pct.min(40)),
        "░".repeat(40usize.saturating_sub(pct))
    )
}
...
fn main() {
    println!("\n╔════════════════════════════════════════════════════════════════╗");
    println!(
        "║  TX SIZE METER — Solana limit: {} bytes                    ║",
        LIMIT
    );
    println!("╚════════════════════════════════════════════════════════════════╝");
    ...
    println!(
        "  {:<42} {:>4}       {:>42}",
        "Mode", "Size", "[░░░░░░░░ bar ░░░░░░░░] size/1232"
    );
    println!("  {:-<106}", "");

    measure(
        "Ví MỚI  (+WSOL ATA +Token ATA)",
        &buyer,
        vec![
            ...
            Instruction {
                program_id: tp,
                accounts: vec![AccountMeta::new(u_wsol_ata, false)],
                data: vec![17],
            },
