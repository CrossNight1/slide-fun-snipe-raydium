...
pub async fn raydium_bundle_buy(
    ...
    for attempt in 0..=MAX_RETRIES {
        ...
        // --- FORCE RPC SPAM PATH (No Jito) ---
        log_info!("[BUNDLE] Sending individual TXs via RPC (No Jito Mode)");
        ...
        for (idx, tx) in buy_txs.iter().enumerate() {
            ...
            // SPAM: Send each transaction 5 times to ensure it lands
            for spam_i in 1..=5 {
                let rpc_cc = rpc_c.clone();
                let tx_cc = tx_c.clone();
                let sent_cc = sent_c.clone();
                let trades_cc = trades_c.clone();
                let mint_cc = mint_c.clone();
                let wallet_cc = wallet_c.clone();
                let wtype_cc = wtype_c.clone();
                let network_cc = network_c.clone();
                let wi_val = wi;
                ...
                let h = tokio::spawn(async move {
                    ...
                    match rpc_cc.send_transaction_with_config(&tx_cc, config_rpc).await {
                        Ok(sig) => {
                            let cluster = if network_cc.to_lowercase() == "devnet" { "?cluster=devnet" } else { "" };
                            log_info!("[BUNDLE] RPC TX OK (Attempt {}): {}{}", spam_i, sig, cluster);
                            sent_cc.lock().await.insert(wi_val);
                            
                            if spam_i == 1 {
                                trades_cc.add_trade(Trade {
                                    id: sig.to_string(),
                                    ...
                                    latency_ms: None,
                                    wallet: Some(wallet_cc),
                                    wallet_type: Some(wtype_cc),
                                }).await;
                            }
                        }
                        Err(_) => {} // Silent fail on spam
                    }
                });
                handles.push(h);
...
pub async fn build_raydium_sell_tx_for_wallet(
    ...
    let base_token_program = pool_info.base_token_program;
    let wsol_mint = Pubkey::from_str(constants::WSOL_MINT).unwrap();
    let tp = Pubkey::from_str(constants::TOKEN_PROGRAM).unwrap();
    ...
    let ixs = vec![
        ...
        create_associated_token_account_idempotent(&user, &user, &wsol_mint, &tp),
        swap_ix,
...
pub async fn raydium_bundle_sell(
    ...
    for tx in sell_txs.into_iter() {
        match rpc.send_transaction_with_config(&tx, solana_client::rpc_config::RpcSendTransactionConfig {
            ...
            Ok(sig) => log_info!("[BUNDLE] Raydium SELL OK: {}", sig),
            Err(e) => log_info!("[BUNDLE] Raydium SELL Error: {:?}", e),
        }
    }
    log_info!("[BUNDLE] ✅ Done");
}
    ...
    for attempt in 0..=MAX_RETRIES {
        ...
        for &wi in &pending {
            ...
            match build_slidefun_buy_tx_for_wallet(
                keypair, &token_mint_pk, &tp_arg, fee_to, &slidefun_program, sol_lamports,
                config.cu_limit + attempt as u32, config.priority_fee, current_bh,
            ) {
        ...
        log_info!("[BUNDLE] Sending individual Slide.fun TXs via RPC (No Jito Mode)");
        let sent_set: Arc<tokio::sync::Mutex<std::collections::HashSet<usize>>> =
            Arc::new(tokio::sync::Mutex::new(std::collections::HashSet::new()));
        ...
        for (idx, tx) in buy_txs.iter().enumerate() {
            ...
            let wi = pending[idx];
            let trades_c = trades.clone();
            let mint_c = token_mint_pk.to_string();
            let sol_c = all_wallets[wi].1;
            let wallet_c = all_wallets[wi].0.pubkey().to_string();
            let wtype_c = all_wallets[wi].2.to_string();
            let sent_c = sent_set.clone();
            ...
            for spam_i in 1..=5 {
                ...
                let h = tokio::spawn(async move {
                    ...
                    match rpc_cc.send_transaction_with_config(&tx_cc, config_rpc).await {
                        Ok(sig) => {
                            log_info!("[BUNDLE] SF RPC TX OK (Attempt {}): {}", spam_i, sig);
                            sent_cc.lock().await.insert(wi_val);
                            if spam_i == 1 {
                                trades_cc.add_trade(Trade {
                                    id: sig.to_string(),
...
pub async fn slidefun_bundle_sell(
    ...
    for tx in sell_txs.into_iter() {
        match rpc.send_transaction_with_config(&tx, solana_client::rpc_config::RpcSendTransactionConfig {
            ...
            Ok(sig) => log_info!("[BUNDLE] SF SELL OK: {}", sig),
            Err(e) => log_info!("[BUNDLE] SF SELL Error: {:?}", e),
        }
    }
    log_info!("[BUNDLE] ✅ Done");
